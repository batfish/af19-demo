...
def test_no_ibgp_sessions(bf):
    os.environ['bf_policy_name'] = "Routing and Routing Protocol Policies"
    bf.asserts.current_assertion = 'Assert no iBGP sessions configured in the fabric'

    ibgp_sessions = bf.q.bgpSessionCompatibility(type='/IBGP.*/').answer().frame()[['Node', 'Local_AS', 'Remote_Node', 'Remote_AS']]

    try:
        assert len(ibgp_sessions.index)==0
        record_results(bf, status=TEST_STATUS_PASS, message='All routers only have eBGP sessions')
...
def test_no_duplicate_bgp_asn(bf):
    os.environ['bf_policy_name'] = "Routing and Routing Protocol Policies"
    bf.asserts.current_assertion = 'Assert all routers have an unique BGP ASN'

    bgpProc = bf.q.bgpPeerConfiguration(properties='/Local_AS/').answer().frame()
    asn_list = set(bgpProc.Local_AS)

    dup_asn_list = []
    found_dup_asn = False
    for asn in asn_list:
        nodes = bgpProc[bgpProc['Local_AS'] == asn]
        node_set = set(nodes['Node'])
        if len(node_set) != 1:
            t_dict = {'BGP_AS': asn, 'Nodes': node_set}
            dup_asn_list.append(t_dict)
            found_dup_asn = True

    try:
        assert found_dup_asn==False
        record_results(bf, status=TEST_STATUS_PASS, message='All routers have a unique BGP ASN')
    except Exception as e:
        record_results(bf, status=TEST_STATUS_FAIL,
                                  message='Routers with duplicate BGP ASNs:\n{}'.format(dup_asn_list))
        raise e
...
def test_all_leaf_have_route_for_all_host_subnets(bf):
    ...
    try:
        ...
        record_results(bf, status=TEST_STATUS_PASS, message='All host subnets are present on all leaf routers')
    except Exception as e:
        record_results(bf, status=TEST_STATUS_FAIL,
...
def test_no_duplicate_router_ids(bf):
    ...
    rtr_id_map = bgpProc[['Node', 'Router_ID']]

    dup_rtr_id_df = rtr_id_map[rtr_id_map.duplicated(['Router_ID'], keep=False)]

    try:
        assert len(dup_rtr_id_df.index)==0
        record_results(bf, status=TEST_STATUS_PASS, message='All routers have a unique router-id')
    except Exception as e:
        record_results(bf, status=TEST_STATUS_FAIL,
...
def test_leaf_spine_bgp_peers(bf):
    ...
    try:
        ...
        record_results(bf, status=TEST_STATUS_PASS, message='All leaf routers have BGP sessions configured for each spine')
    except Exception as e:
        record_results(bf, status=TEST_STATUS_FAIL,
